...
export class ChatService {
  ...
  async addMessage(input: {
    ...
    return prisma.chatMessage.create({
      data: {
        ...
        citations: input.citations === undefined ? undefined : JSON.parse(JSON.stringify(input.citations)),
        toolEvents: input.toolEvents === undefined ? undefined : JSON.parse(JSON.stringify(input.toolEvents)),
      },
    });
  }
  ...
  async generateReply(
    ...
    if (this.ragService.containsPromptInjection(lastUserMessage)) {
      ...
      return {
        content: "I don't know. The request appears to ask me to ignore instructions or reveal protected context.",
        citations: [],
        toolEvents: [],
      };
    }
    ...
    if (!client) {
      ...
      if (!chunks.length || (chunks[0]?.score ?? 0) < MIN_RELEVANCE_SCORE) {
        return { content: "I don't know based on the documents in this workspace.", citations: [], toolEvents: [] };
      }
    ...
    while (iterationCount < MAX_TOOL_ITERATIONS) {
      ...
      if (functionCalls && functionCalls.length > 0) {
        const functionResults: Array<{ functionResponse: { name: string; response: unknown } }> = [];
        ...
        for (const call of functionCalls) {
          ...
          try {
            ...
            toolEvents.push({ tool: toolName, input: toolInput, error: errorMessage, status: 'error' });
            functionResults.push({
        ...
        const toolResultMessage = functionResults.map((r) => ({
          functionResponse: r.functionResponse,
        }));
        currentMessage = JSON.stringify(toolResultMessage);

        // Use sendMessage with function results
    ...
    const usageMetadata = chunks.length > 0 ? { retrievedChunks: chunks.length, latencyMs } : { latencyMs };
    logger.info({ workspaceId, latencyMs, retrievedChunks: chunks.length, toolCallCount: toolEvents.length }, 'chat reply generated');
  ...
  async *streamReplyTokens(
    ...
    if (this.ragService.containsPromptInjection(lastUserMessage)) {
      yield { type: 'token', data: "I don't know. The request appears to ask me to ignore instructions or reveal protected context." };
    ...
    if (!client) {
      ...
      const context = chunks.map((c: RetrievedChunk, i: number) => `[${i + 1}] ${c.citation}\n${c.content}`).join('\n\n');
    ...
    for await (const chunk of streamResult.stream) {
      ...
      if (functionCalls && functionCalls.length > 0) {
        for (const call of functionCalls) {
          try {
            const output = await executeTool(call.name, call.args as Record<string, unknown>, { userId, workspaceId });
            yield { type: 'tool_event', data: { tool: call.name, output, status: 'success' } };
          } catch (error) {
            yield { type: 'tool_event', data: { tool: call.name, error: error instanceof Error ? error.message : 'failed', status: 'error' } };
          }
        }
      }
    }
